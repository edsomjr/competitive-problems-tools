...
namespace cptools::task
{
    ...
    namespace files
    {
        const std::filesystem::path solution_executable(std::string(CP_TOOLS_BUILD_DIR)
                                                        + "/solution");
        const std::filesystem::path generator_executable(std::string(CP_TOOLS_BUILD_DIR)
                                                         + "/generator");
    } // namespace files
