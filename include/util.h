...
namespace cptools::util
{
    ...
    template <typename T>
    nlohmann::json
    create_json_operation(const std::string &json_pointer, const std::string &op, T new_value)
    {
    ...
    template <typename T>
    T
    get_json_value(const nlohmann::json &config, const std::string &fields, T default_value)
    {
