...
int judge(const std::string &solution_path, std::ostream &out,
  ...
  string error;
  auto rc =
      task::build_tools(error, task::tools::VALIDATOR | task::tools::CHECKER);
  ...
  if (rc != CP_TOOLS_OK) {
    err << message::failure("Error on solution '" + solution_path +
                            "' compilation")
        << '\n';
    err << message::trace(error) << '\n';
    return verdict::CE;
  ...
  auto memory_limit =
      cptools::util::get_json_value(config, "problem|memory_limit", 1000);
  ...
  out << format::apply("Verdict:", format::emph::BOLD + format::align::LEFT,
                       col_size)
      << format::apply(ver_string[ans], ver_style.at(ans) + format::align::LEFT)
      << '\n';

  out << format::apply("Passed:", format::emph::BOLD + format::align::LEFT,
                       col_size)
      << format::apply(to_string(passed), format::style::INT) << '\n';

  out << format::apply("Max time:", format::emph::BOLD + format::align::LEFT,
                       col_size)
      << format::apply(as_string(tmax, 6), format::style::FLOAT) << '\n';

  out << format::apply("Max memory:", format::emph::BOLD + format::align::LEFT,
                       col_size)
