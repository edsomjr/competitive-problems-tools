...
namespace cptools::commands::judge
    {
        ...
        const std::map<int, long long> ver_style{
            { verdict::AC, cli::format::style::AC },
            { verdict::PE, cli::format::style::PE },
            { verdict::WA, cli::format::style::WA },
            { verdict::CE, cli::format::style::CE },
            { verdict::TLE, cli::format::style::TLE },
            { verdict::RTE, cli::format::style::RTE },
            { verdict::MLE, cli::format::style::MLE },
            { verdict::FAIL, cli::format::style::FAIL },
        ...
        int judge(const std::string &solution_path)
        {
            ...
            if (not res.ok)
                {
                    cli::write(cli::fmt::error,
                               "Could not generate executable: " + res.error_message);
                    return verdict::CE;
                }
                    ...
                    if (ver == verdict::AC)
                        {
                            ...
                            res = sh::execute_program(checker, args, "", "/dev/null",
                                                      2 * time_limit / 1000.0);
            ...
            oss << cli::format::apply(
                "Verdict:", cli::format::emph::BOLD + cli::format::align::LEFT, col_size)
            ...
            oss << cli::format::apply(
                "Max time:", cli::format::emph::BOLD + cli::format::align::LEFT, col_size)
                << cli::format::apply(as_string(tmax, 6), cli::format::style::FLOAT) << std::endl;

            oss << cli::format::apply(
                "Max memory:", cli::format::emph::BOLD + cli::format::align::LEFT, col_size)
