...
int validate_checker(std::ostream &out, std::ostream &err) {
  ...
  out << message::info("Testing the checker (" + to_string(tests.size()) +
                       " tests) ...\n");
...
int validate_validator(std::ostream &out, std::ostream &err) {
  ...
  out << message::info("Testing the validator (" + to_string(tests.size()) +
                       " tests) ...\n");
  ...
  for (auto [input, verdict] : tests) {
    ...
    string res =
        (result.output.find("FAIL") == string::npos ? "OK" : "INVALID");

    if (verdict != res) {
      err << message::failure("Input '" + input + " is invalid: expected = '" +
                              verdict + "', got = '" + res + "'\n");
      err << message::trace(result.output.empty() ? "Test valid!"
                                                  : result.output)
          << '\n';
      return result.rc;
    }
...
int validate_tests(std::ostream &out, std::ostream &err) {
  ...
  out << message::info("Validating the input files (" +
                       to_string(io_files.size()) + " tests) ...\n");
