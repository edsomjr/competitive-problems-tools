...
static struct option longopts[] = {{"help", no_argument, NULL, 'h'},
                        ...
                        const std::string &problem_id, bool forced)
    {
        ...
        auto new_file_path = conflicts::solve_files(local_file_name, polygon_file_name,
                                                    polygon_file_content, forced);
        ...
        for (const auto &solution : solutions)
            {
                ...
                const auto remote_file_name
                    = std::filesystem::path(solution.name).filename().string();
                ...
                auto file_exists = std::find(files_with_same_tag.begin(), files_with_same_tag.end(),
                                             remote_file_name)
                                   != files_with_same_tag.end();

                if (!file_exists)
                    fs::overwrite_file(remote_file_path, file_content);
                else
                    conflicts::solve_files(remote_file_path, remote_file_name, file_content,
                                           forced);
    ...
    int run(int argc, char *const argv[])
    {
        ...
        try
        ...
        catch (const exceptions::polygon_api_error &e)
            {
                ...
                if (not forced)
                    {
                        cli::write(cli::fmt::warning, "Recovering " + config::config_path_name
