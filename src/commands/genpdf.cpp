...
int generate_pdf(const string &doc_class, const string &language, int flags,
  ...
  auto task =
      tutorial ? gentex::generate_tutorial_latex : gentex::generate_latex;
...
int run(int argc, char *const argv[], ostream &out, ostream &err) {
  ...
  while ((option = getopt_long(argc, argv, "ho:c:lg:b:t", longopts, NULL)) !=
         -1) {
    switch (option) {
    case 'h':
      out << help() << '\n';
    ...
    case 'g': {
      ...
      if (not gentex::validate_language(language)) {
        err << message::failure("Language '" + language +
