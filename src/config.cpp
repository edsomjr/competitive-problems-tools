...
namespace cptools::config
{
    ...
    read_config_file()
    {
        return util::read_json_file(config_path_name);
    }

    std::string
    get_polygon_problem_id()
    {
    ...
    std::string
    get_tool_file_name(const std::string &type)
    {
    ...
    std::string
    get_tool_file_name(const tool_type type)
    {
        const auto json_object = read_config_file();
     ...
     */
    void
    insert_solution_file_name(const std::string &tag, const std::string &file_name)
    {
        const auto json_object = read_config_file();
    ...
    get_solutions_file_paths(const std::string &tag, const bool filename_only)
    {
    ...
    std::vector<std::string>
    get_tests_file_names(const test_type type)
    {
        const auto config_json = read_config_file();
        auto path = "tests|" + test_type_tag.at(type);
        auto tests = util::get_json_value<std::unordered_map<std::string, std::string> >(
            config_json, path, {});
