...
std::string get_tool_file_name(const nlohmann::json &json_object,
...
get_solutions_file_names(const nlohmann::json &json_object,
  ...
  if (tag == "default") {
    ...
    if (file_name == "")
      throw(
          exceptions::invalid_config_error("No valid default solution found"));
    return std::vector<std::string>{file_name};
  }

  auto file_names =
      util::get_json_value<std::vector<std::string>>(json_object, path, {});
  return file_names;
}
