...
namespace cptools::fs
{
    ...
    const Result
    copy(const std::string &src, const std::string &dst, bool overwrite)
    {
    ...
    const Result
    equivalent(const std::string &p1, const std::string &p2)
    {
        bool same = false;
        try
            {
    ...
    const Result
    rename(const std::string old_path, const std::string new_path)
    {
        try
            {
    ...
    void
    overwrite_file(const std::string dst, const std::string content)
    {
        std::ofstream file;
        file.open(dst);
        file << content;
     ...
     */
    std::string
    sha_512_file(std::string filename)
    {
